...
thread_exit (void)
{
  ...
  DEBUG_thread_prepare_exit();
  map_remove_if(&thread_current()->open_files); //VÅRT BIDRAG
...
/* DEBUG code added by klaar@ida, modified by filst@ida */
#include "init.h"
static unsigned DEBUG_thread_alive_count;        /* Number of threads alive. */
static unsigned DEBUG_thread_pending_exit;       /* Number of threads exiting process_cleanup. */
static struct lock DEBUG_thread_alive_lock;      /* Lock for the two variables above. */
static struct condition DEBUG_thread_alive_cond; /* Condition for waiting for changes. */

void DEBUG_thread_init(void)
{
  DEBUG_thread_alive_count = 1; // initial (main) thread
  DEBUG_thread_pending_exit = 0;
  lock_init(&DEBUG_thread_alive_lock);
  cond_init(&DEBUG_thread_alive_cond);
}

void DEBUG_thread_created(void)
{
...
void DEBUG_thread_prepare_exit(void)
{
  lock_acquire(&DEBUG_thread_alive_lock);
  ++DEBUG_thread_pending_exit;
  lock_release(&DEBUG_thread_alive_lock);
}

void DEBUG_thread_exited(void)
{
  lock_acquire(&DEBUG_thread_alive_lock);
  --DEBUG_thread_pending_exit;
  --DEBUG_thread_alive_count;
  cond_broadcast(&DEBUG_thread_alive_cond, &DEBUG_thread_alive_lock);
  lock_release(&DEBUG_thread_alive_lock);
}
...
{
  lock_acquire(&DEBUG_thread_alive_lock);
  /* Wait until only the initial thread and the idle thread remain.
     Don't emit any messages if DEBUG_thread_pending_exit is > 0,
     that means threads are running process_cleanup and will exit soon. */
  while ( DEBUG_thread_alive_count > 2 )
    {
      if ( DEBUG_thread_pending_exit > 0 )
        {
          /* Don't emit messages, wait for the thread(s) to finish running
             process_cleanup() before considering doing that. */
        }
      else if ( thread_current() == initial_thread )
        {
