...
static void
trap_interrupt (struct intr_frame *args UNUSED)
{
  /* Just calling the trap is slow enough. We could spin on a call to barrier () here if we need slower execution. */
}
...
static void
trap_syscall (struct intr_frame *args)
{
  slowdown_enable ();
...
void
slowdown_enable (void)
{
  if (!slowdown_active)
    return;

  /* Set the trap flag so that we get an interrupt on every instruction. Only applicable to the current thread. */
  asm volatile ("pushfl; movl %%esp, %%eax; orl $0x0100, (%%eax); popfl;" : : : "eax" );
}

/* Disable slowdown for this thread. */
void
slowdown_disable (void)
{
  if (!slowdown_active)
    return;

  /* Set the trap flag so that we get an interrupt on every instruction. Only applicable to the current thread. */
  asm volatile ("pushfl; movl %%esp, %%eax; andl $0xFFFFFEFF, (%%eax); popfl;" : : : "eax" );
}

/* Check if slowdown is enabled for this thread. */
bool
slowdown_enabled (void)
{
  int flags;
